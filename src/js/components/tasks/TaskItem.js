...
class TaskItem extends Component {
    ...
    render() {
        ...
        const item = this.props.listTasks[this.props.selectedTask].listItems[this.props.selectedItem];
        return connectDropTarget(connectDragSource(
            <div className={item.completed ? "task-item completed" : "task-item"}>
                <h5>{item.name}</h5>
                <div className="task-item-tick" onClick={this.onClick.bind(this, this.props.selectedTask, this.props.selectedItem)}>&#10004;</div>
            </div>
        ));
    }
}
...
TaskItem = DragSource(Item_Types.TASK_ITEM, taskItemSource, (connect, monitor) => ({
...
export default connect(mapStateToProps, { tickItem, beginDrag, dropItem })(TaskItem);