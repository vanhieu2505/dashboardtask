  ...
  switch (action.type) {
    ...
    case SHOW_BOARD:
      return {
        ...state,
        type: action.type,
        selectedBoard: state.listBoards.indexOf(
    ...
    case TICK_ITEM:
      const updatedBoardsForTickItem = state.listBoards.map((board, index) => {
        if (index === state.selectedBoard) {
          const updatedTasksForTickItem = board.listTasks.map(
            (task, subIndex) => {
              if (subIndex === action.selectedTask) {
                const updatedItemsForTick = task.listItems.map(
                  (item, tickItem) => {
    ...
    case DROP_ITEM:
      const updatedBoardForDropItem = state.listBoards.map(
        (board, boardIndex) => {
          if (boardIndex === state.selectedBoard) {
            const updatedBoardForDropItem = board.listTasks.map(
              (task, taskIndex) => {
                //drag and drop on same task
                if (action.selectedTask === state.draggedSelectedTask) {
                  if (taskIndex === state.draggedSelectedTask) {
                    const draggedItem =
                      board.listTasks[state.draggedSelectedTask].listItems[
                        state.draggedSelectedItem
                      ];
                    let updatedListItemPopin = [
                      ...task.listItems.slice(0, action.selectedItem),
                      {
                    ...
                    if (action.selectedItem > state.draggedSelectedItem) {
                      updatedListItemPopin = updatedListItemPopin.filter(
                        (itemOut, itemOutIndex) =>
                          itemOutIndex !== state.draggedSelectedItem
                      );
                    } else {
                      //if popin index is lesser than popout index, have to increase popout index by 1
                      updatedListItemPopin = updatedListItemPopin.filter(
                        (itemOut, itemOutIndex) =>
                          itemOutIndex !== state.draggedSelectedItem + 1
                      );
                    }
                  ...
                  if (taskIndex === state.draggedSelectedTask) {
                    const updatedListItemPopout = task.listItems.filter(
                      (itemOut, itemOutIndex) =>
                        itemOutIndex !== state.draggedSelectedItem
                    );
                    return {
